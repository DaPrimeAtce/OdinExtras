...
object ChatCommandsPlus : Module(
    ...
    private val warp by StringSetting("Warp", "!warp, !w", 128, desc = "Executes the /party warp command.") .withDependency { stringSettings }
    private val coords by StringSetting("Coords", "!coords, !co", 128, desc = "Sends your current coordinates.").withDependency { stringSettings }
    private val allinv by StringSetting("All Invite", "!ai, !allinv, !allinvite", 128, desc = "Executes the /party settings allinvite command.").withDependency { stringSettings }
    private val boop by StringSetting("Boop", "!boop", 128, desc = "Executes the /boop command.").withDependency { stringSettings }
    private val kick by StringSetting("Kick", "!kick, !k", 128, desc = "Executes the /p kick command.") .withDependency { stringSettings }
    private val cf by StringSetting("Coinflip", "!cf, !coinflip", 128, desc = "Sends the result of a coinflip.").withDependency { stringSettings }
    private val transfer by StringSetting("Transfer", "!pt, !ptme, !transfer", 128, desc =  "Executes the /party transfer command.") .withDependency { stringSettings }
    private val reinv by StringSetting("Reinvite", "!reinv, !reinvite", 128, desc = "Reinvites the player who sent it a few seconds later.") .withDependency { stringSettings }
    private val ping by StringSetting("Ping", "!ping", 128, desc = "Sends your current Ping.").withDependency { stringSettings }
    private val tps by StringSetting("TPS", "!tps", 128, desc = "Sends your server's current TPS.").withDependency { stringSettings }
    private val fps by StringSetting("FPS", "!fps", 128, desc = "Sends your current FPS.").withDependency { stringSettings }
    private val dt by StringSetting("DT", "!downtime, !dt", 128, desc = "Sets a reminder for the end of the run and cancels auto requeue.").withDependency { stringSettings }
    private val undt by StringSetting("UnDT", "!undowntime, !undt", 128, desc = "Undoes a downtime command from earlier in a run.").withDependency { stringSettings }
    private val inv by StringSetting("Invite", "!invite, !inv", 128, desc = "Invites the player to your party.") .withDependency { stringSettings }
    private val time by StringSetting("Time", "!time", 128, desc = "Sends the current time.").withDependency { stringSettings }
    private val demote by StringSetting("Demote", "!demote", 128, desc = "Executes the /party demote command.").withDependency { stringSettings }
    private val promote by StringSetting("Promote", "!promote", 128, desc = "Executes the /party promote command.").withDependency { stringSettings }
    private val kickOffline by StringSetting("Kick Offline", "!kickoffline, !ko", 128, desc = "Allows you to kick offline players.").withDependency { stringSettings }
    private val location by StringSetting("Location", "!location", 128, desc = "Sends your current location.").withDependency { stringSettings }
    private val holding by StringSetting("Holding", "!holding", 128, desc = "Sends the item you are holding.").withDependency { stringSettings }
    private val disband by StringSetting("Disband", ".disband", 128, desc = "Disbands the party.") .withDependency { stringSettings }
    ...
    fun registerChatCommandsOnInitializeClient() {
        commands = listOf(
            ...
            ChatCommand({ inv }, channelsOf(ChatChannel.PRIVATE, ChatChannel.COOP),  false) { _, name, _ ->
            ...
            ChatCommand({ coords }, channelsOf(ChatChannel.PARTY, ChatChannel.PRIVATE, ChatChannel.COOP), false) { _, name, channel ->
                channelMessage(getPositionString(), name, channel)
            },
            ChatCommand(
                { boop }, channelsOf(ChatChannel.PARTY, ChatChannel.PRIVATE,
                ChatChannel.GUILD, ChatChannel.COOP), false) { words, name, _ ->
            ...
            ChatCommand(
                ...
                ChatChannel.GUILD, ChatChannel.COOP), false) { _, name, channel ->
    ...
    init {
        ...
        on<MessageSentEvent> {
            if (!moreChatEmotes || (message.startsWith("/") && !listOf("/pc", "/ac", "/gc", "/msg", "/w", "/r", "/cc").any { message.startsWith(it) })) return@on
